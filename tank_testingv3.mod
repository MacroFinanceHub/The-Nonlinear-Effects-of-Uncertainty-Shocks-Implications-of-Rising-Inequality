...
var y_til pi i_hat y_hat rn_hat nu z a sigma_a sigma_z sigma_nu;
// Declare exogenous variables
varexo e_nu e_z e_a e_siga e_sigz e_signu;
// Declare parameters
parameters beta sigma psi epsilon_p zeta delta tri rho_nu rho_z rho_a phi_pi phi_y tau lambda Mu_p omega kappa Psi Del_p_pi gamma Phi psi_a sd_e_nu sd_e_z sd_e_a rho_siga rho_sigz rho_signu sigma_siga sigma_sigz sigma_signu sd_e_siga sd_e_sigz sd_e_signu;
//prefs/technology
beta=0.9745;
sigma = 1;
psi = 1;
...
// Declare TANK model-- eqns 13-15 in note plus eqns to add uncertainty 24-25 in note
//model(linear);
model;
...
i_hat = phi_pi*pi + phi_y*y_hat + sigma_nu(-1)*nu;
nu = rho_nu*nu(-1)+ sigma_nu(-1)*e_nu;
